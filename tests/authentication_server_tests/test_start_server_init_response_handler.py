...
class StartServerInitResponseHandlerTest(unittest.TestCase):
	...
	def test_handle_message(self):
		...
		global_data.server_name = 'authentication_server'
		channel_buffer = Mock()
		message = protocol.protocol_message_pb2.EndServerInitNotification()
		message.name = 'authentication_server'		
		
		self.handler.handle_message(global_data, 'test_channel', ProtocolID.START_SERVER_INIT_RESPONSE, channel_buffer)
		
		global_data.rmq.subscribe.assert_called_with('server_status')
		global_data.rmq.send_message_string.assert_called_with(message, 'server_initialization', ProtocolID.END_SERVER_INIT_NOTIFICATION)
